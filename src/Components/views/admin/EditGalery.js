...
const EditGallery = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/gallery/postsGallery/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/gallery/postsGallery/${id}`, formData, {
        headers: {
